...
// SAMPLE DATA
const current = {temp:74,condition:"Storm",dew:65,humidity:57,pressure:28.97}
...
const forecast = [{high:70,low:44,cond:"Storm"},{high:62,low:45,cond:"Storm"},{high:68,low:50,cond:"Storm"}]
...
function draw(){
    ...
    // TEMP RIGHT OF ICON WITH TENTHS
    gradientText("#FFE0E0","#FFC8C8", col1+150, row1+120, current.temp.toFixed(1)+"°", 150)
    // CONDITION TEXT BELOW TEMP
    ctx.font="28px Arial"
    ctx.fillStyle="#FFF"
    ctx.fillText(current.condition,col1+150,row1+160)
    // LOWER DATA ROW
    ctx.font="22px Arial"
    ctx.fillText("Dew Point",col1+20,row1+230)
    ctx.fillText("Humidity",col1+150,row1+230)
    ctx.fillText("Pressure",col1+340,row1+230)
    gradientText("#A0FFD0","#D0FFEE", col1+20,row1+260,current.dew+"°",28)
    gradientText("#A0C8FF","#D0E0FF", col1+150,row1+260,current.humidity+"%",28)
    let mb=Math.round(current.pressure*33.8639)
    gradientText("#FFA0FF","#FFD0FF", col1+340,row1+260,`${current.pressure.toFixed(2)}" / ${mb} MB`,28)
    ...
    ctx.fillText("Direction",col2+20,row2+170)
    gradientText("#FFC080","#FFD0AA",col2+120,row2+110,wind.speed,30)
    gradientText("#80D0FF","#40A0FF",col2+120,row2+140,wind.gust,30)
    gradientText("#FFD0AA","#FFA080",col2+120,row2+170,wind.direction+"°",30)
    // WIND ARROW
    const cx=col2+350, cy=row2+140
    ...
    // Mini temperature graph since midnight
    let px = col2+40, py=row3+160, pw=colWidth-80, ph=30
    let low=todayStats.low, high=todayStats.high
    let hourlyTemps = [51,52,53,55,57,60,63,68,72,74] // fake data
    ctx.beginPath()
    ...
    let gradient = ctx.createLinearGradient(px, py, px, py+ph)
    gradient.addColorStop(0,'#FFC080')
    gradient.addColorStop(1,'#FFD0AA')
